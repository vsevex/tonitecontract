...
export class CoordinatorUnit implements Contract {
  ...
  static createFromOwnerAndKey(
    owner: Address,
    publicKeyEcvrf: bigint,
    publicKeyReplay: Buffer
  ) {
  ...
  async sendProvideRandomness(
    ...
    const signature = beginCell()
      .storeBuffer(sign(hashToSign, secretReplay))
      .endCell();
    await provider.external(
      beginCell().storeSlice(pi).storeRef(signature).endCell()
    );
  }

  async getCalcPiFromAlpha(
    provider: ContractProvider,
    secret: bigint,
    alpha: Slice
  ): Promise<Slice> {
    ...
    const result = await provider.get(
      "ecvrf::rist255::with_secret::prove",
      args.build()
    );
  ...
  async getPublicKey(
    ...
    args.writeNumber(secret);
    return (
      await provider.get("rist255::get_public_key", args.build())
    ).stack.readBigNumber();
  }
