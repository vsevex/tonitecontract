...
describe('Coordinator test cases', () => {
    ...
    beforeEach(async () => {
        ...
        coordinator = blockchain.openContract(
            CoordinatorUnit.createFromOwnerAndKey(deployer.address, secretEcvrf, keyReplay.publicKey),
        );
    ...
    it('create from owner and key', async () => {
        const sameContract = CoordinatorUnit.createFromOwnerAndKey(deployer.address, secretEcvrf, keyReplay.publicKey);
        expect(sameContract.address.toString()).toEqual(coordinator.address.toString());

        const differentOwner = CoordinatorUnit.createFromOwnerAndKey(owner, secretEcvrf, keyReplay.publicKey);
        expect(differentOwner.address.toString()).not.toEqual(coordinator.address.toString());
    });

    it('should send subscribe random', async () => {
        const consumer = randomAddress();

        const result = await coordinator.sendSubscribeRandom(deployer.getSender(), toNano('2.0'), consumer);
