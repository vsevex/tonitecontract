...
describe("Tonite", () => {
  ...
  beforeEach(async () => {
    ...
    const deployResult = await tonite.sendDeploy(
      deployer.getSender(),
      toNano("0.01")
    );
  ...
  it("should return participant with a correct address", async () => {
    ...
    const participant = await tonite.getParticipantWithAddr({
      poolId: 121,
  ...
  it("must return a message when trying to join a non-existing pool", async () => {
    const result = await tonite.sendJoinPool(
      deployer.getSender(),
      toNano("0.01"),
      {
        poolId: 121,
  ...
  it("should throw an exception when trying to create with an existing pool id", async () => {
    ...
    const result = await tonite.sendCreatePool({
      seqno: 13,
      poolId: poolId,
      body: poolCell,
    });
  ...
  it("should return 47 when user attempts to join a closed pool", async () => {
    ...
    const joinResult = await tonite.sendJoinPool(
      staker.getSender(),
      toNano("1"),
      {
        poolId: 121,
    ...
    expect(outMessage?.info.dest?.toString()).toEqual(
      staker.address.toString()
    );
    expect(outMessage?.info.type).toEqual("internal");
  ...
  it("should add reward to the cell properly", async () => {
    ...
    const rewardFirst = await tonite.getRewardWithKey({
      poolId: poolId,
      key: 1,
    });
    const rewardSecond = await tonite.getRewardWithKey({
      poolId: poolId,
      key: 2,
    });
    const rewardForth = await tonite.getRewardWithKey({
      poolId: poolId,
      key: 4,
    });
